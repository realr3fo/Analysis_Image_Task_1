...
original_img = cv2.imread("./dataset/1660028400.jpg", cv2.IMREAD_COLOR)
img_bg = cv2.imread("./dataset/1660021200.jpg", cv2.IMREAD_COLOR)

resized_img = cv2.resize(original_img, (0, 0), fx=0.8, fy=0.8)
roi = resized_img[350:, :]

resized_img_bg = cv2.resize(img_bg, (0, 0), fx=0.8, fy=0.8)
roi_bg = resized_img_bg[350:, :]
...
# process upper right
roi_copy = roi.copy()
upper_right = roi_copy[:200, 1000:]
upper_right_bg = roi_bg[:200, 1000:]
upper_right_subtracted = cv2.subtract(upper_right_bg, upper_right)
# remove shadow
lower_shadow = np.array([0, 0, 0])
upper_shadow = np.array([70, 80, 80])
...
upper_right_thresh = binarize_by_adaptive(upper_img_gray, 13, 1, (3, 3))
kernel3 = np.ones((2, 2), dtype=np.uint8)
kernel4 = np.ones((5, 5), dtype=np.uint8)
upper_morphed = upper_right_thresh
# upper_morphed = cv2.morphologyEx(upper_morphed, cv2.MORPH_DILATE, kernel4)
# upper_morphed = cv2.morphologyEx(upper_morphed, cv2.MORPH_CLOSE, kernel4)
# upper_morphed = cv2.morphologyEx(upper_morphed, cv2.MORPH_ERODE, kernel4)
